...
func init() {
	...
	if ok {
		DBTempPath = path.Join(path.Dir(filename), "./ipfilter-GeoLite2-Country.mmdb.gz")
	}
}
...
func (f *IPFilter) initDB() error {
	_, filename, _, ok := runtime.Caller(1)
	if !ok {
		return errors.New("Unable to identify current file path.")
	}
	if len(f.opts.IPDB) > 0 {
		...
		//local path
		file, err := os.Open(path.Join(path.Dir(filename), f.opts.IPDBPath))
		if err != nil {
			return err
		}
		defer file.Close()
